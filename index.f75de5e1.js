...
const updateButton = ()=>{
    ...
    const colCount = table.querySelector("tr:first-child").querySelectorAll("td").length;
...
addColumn.addEventListener("click", ()=>{
    if (table.querySelector("tr:first-child").querySelectorAll("td").length < MAX_COUNT) for(let i = 0; i < table.rows.length; i++){
...
deleteColumn.addEventListener("click", ()=>{
    if (table.querySelector("tr:first-child").querySelectorAll("td").length > MIN_COUNT) for(let i = 0; i < table.rows.length; i++){
        const row = table.rows[i];
        row.deleteCell(-1);
        updateButton();
    }
